...
StereoCalib(const vector<string>& imagelist, Size boardSize, float squareSize, bool displayCorners = false, bool useCalibrated = true, bool showRectified = true)
{
	...
	FileStorage fs("stereo_image/intrinsics.yml", FileStorage::WRITE);
	if (fs.isOpened())
	{
	...
	fs.open("stereo_image/extrinsics.yml", FileStorage::WRITE);
	if (fs.isOpened())
	{
...
int main(int argc, char** argv)
{
	...
	string imagelistfn;
	bool showRectified;
	cv::CommandLineParser parser(argc, argv, "{w|9|}{h|6|}{s|1.0|}{nr||}{help||}{@input|stereo_image/stereo_calib.xml|}");
	if (parser.has("help"))
		return print_help();
